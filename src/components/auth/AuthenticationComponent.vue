<template>
  <div class="form-control">
    <div class="area-control">
      <input type="text" name="email" id="in-email" required v-model="data.email" :class="['input in-form', {invalid:eValidate}]">
      <label for="in-email" class="marker">Email</label>
      <small>{{eValidate}}</small>
    </div>
    <div class="area-control">
      <input type="password" name="password" id="in-password" required @keypress.enter="enter" v-model="data.password" :class="['input in-form', {invalid:pValidate}]">
      <label for="in-password" class="marker">Пароль</label>
      <small>{{pValidate}}</small>
    </div>
    <div v-if="getUserMessage.status === 422" class="alert" :class="'alert-'+Object.keys(getUserMessage.message)[0]">
      <small>
        <span v-for="message in getUserMessage.message">{{message}}</span>
...
<!--    </label>-->
    <button :class="['sign-in-system btn primary',{load:loader}]" :disabled="loader" type="submit" @click="enter">
      Ввійти <loader v-if="loader" class="type-loader" :radius="8" :width="2"></loader>
    </button>
  </div>
  <div class="form-trip">
    <span class="text">Якщо немає акаунту, <router-link to="/registration" class="text-to">створіть його</router-link></span>
  </div>
</template>
