<template>
    ...
    <div v-else>
        ...
        <div class="card-top">
            ...
            <h4>{{
                    this.$store.getters.getLanguage.types[getTypes.find(type => type.id === getMakeDocument.type_id)?.name]
                }}</h4>
            <!--      <h4>{{getMakeDocument}}</h4>-->
            <span class="rename-title-btn" @click="$emit('prev')">
        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#BBBBBB"
             stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon
            points="14 2 18 6 7 17 3 17 3 13 14 2"></polygon><line x1="3" y1="22" x2="21" y2="22"></line></svg>
      </span>
        </div>
        <div class="fill-areas-document">
            ...
            <div class="fill-areas">
                <div class="form-item"
                     v-for="(item,idx) in getTypes.find(type => type.id === getMakeDocument.type_id)?.fields"
              ...
              <svg xmlns="http://www.w3.org/2000/svg"
                   ...
                   width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#b5b5b5" stroke-width="2"
              ...
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none"
                   stroke="#539DE7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12"
                                                                                                            cy="12"
                                                                                                            r="10"></circle><line
              ...
              <span>{{ this.$store.getters.getLanguage.document_make.hint_description[item] }}</span>
                    ...
                    <component :is="setFields(item)" :key="idx" :field="item" :is-ready="isReady"
                               @catchValidate="validate"></component>
                ...
                <div class="btn-control-panel">
                    ...
                    <button class="button to-archive" @click="toArchive('stop')">
                        {{ this.$store.getters.getLanguage.document_make.signs.save_draft_btn }} та залишитись
                    </button><!--toArchive-->
                    <button class="button to-archive" @click="toArchive('profile')">
                        {{ this.$store.getters.getLanguage.document_make.signs.save_draft_btn }}
                    </button><!--toArchive-->
