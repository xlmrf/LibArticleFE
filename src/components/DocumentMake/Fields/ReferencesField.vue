<template>
    ...
    <span>
      <input type="text" class="sample-input" v-model="reference" @keypress.enter="addReference" ref="referencesPort">
        ...
        <!--          </svg>-->
        <!--        </small>-->
      <span class="add-btn add-ref-btn" @click="addReference">{{
              edit ? $store.getters.getLanguage.document_make.signs.save_btn : $store.getters.getLanguage.document_make.signs.add_btn
          }}</span>
      <span class="search-in-system-btn"
            @click="searchReference">{{ $store.getters.getLanguage.document_make.signs.search_in_lib }}</span>
    </span>
...
<!--        {{ // getMakeDocument.references }}-->
    ...
    <div class="text-error error-area-text border-error" v-if="invalid">
...
export default {
    ...
    methods: {
        ...
        searchReference() {
            ...
            // this.$router.push('/documents?cites_document_id='+this.getMakeDocument.id+'&refs_doc_id='+JSON.stringify(refs_arr))
            localStorage.setItem("cites_documents", JSON.stringify([this.getMakeDocument]))
            this.$router.push('/documents?cites_document_id=' + this.getMakeDocument.id)
